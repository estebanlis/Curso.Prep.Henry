...
function empiezaConNueve(n) {
  ...
  if(Math.abs(p)<1 && Math.abs(p)>0){
    p=p*10;
  }
  if(Math.floor(p) === 9){return true;}
  else{return false;}
...
function mesesDelAño(array) {
  ...
  var array2=[];
  for(let i=0;i<array.length;i++){
    if("Enero"===array[i]){array2.push(array[i]);}
    if("Marzo"===array[i]){array2.push(array[i]);}
    if("Noviembre"===array[i]){array2.push(array[i]);}
  }
